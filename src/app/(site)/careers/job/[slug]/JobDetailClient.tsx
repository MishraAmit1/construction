// src/app/(site)/careers/job/[id]/JobDetailClient.tsx
'use client'
...
export default function JobDetailClient({ jobId, jobSlug, initialJob }: Props) {
    ...
    const generateJobSchema = (job: Job) => {
        ...
        return {
            ...
            "@type": "JobPosting",
            "title": job.title,
            "description": job.job_overview || job.title,
            "identifier": {
                "@type": "PropertyValue",
                "name": "A&T Infracon",
                "value": job.id
            },
            "datePosted": job.posted_on || job.created_at,
            "validThrough": new Date(new Date(job.posted_on || job.created_at).setMonth(new Date(job.posted_on || job.created_at).getMonth() + 3)).toISOString().split('T')[0],
            ...
            "responsibilities": job.key_responsibilities || undefined,
            "skills": job.preferred_skills || undefined,
                ...
                <div className="bg-[#edf3f5] border-b border-gray-200">
                    <div className="container mx-auto px-4 sm:px-6 md:px-12 lg:px-16 xl:px-24 py-4">
                        <nav className="flex items-center text-xs sm:text-sm text-gray-600 font-neuhas tracking-wider">
                            ...
                            <span className="mx-2">&gt;</span>
                            <Link
                                href={`/careers/department/${job.departments?.slug}`}
